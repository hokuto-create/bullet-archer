...
- **装備システム(6部位)**(数値は `CONFIG.equip`、定義は `EQUIP_SLOTS` / `EQUIP_ITEMS`。データ駆動で、
  ...
  - 部位は 武器(攻撃%)/ 兜(最大HP)/ 鎧(被ダメ軽減+高レアはHPも)/ 靴(移動%)/
    ...
    レアリティには装備専用の最上位 **ミシック**(`mythic`)がある(`CONFIG.level.rarity` に weight 0 で
  ...
  - **装備系統(シリーズ)**(定義は `EQUIP_SERIES`。データ駆動で、系統を増やすときは
    ...
    = 龍帝(近距離)/ 月虹(遠距離)/ 陽炎(グレイズ)。**「S級」はレア度ではなく系統の格**(`fxOf` を
    持たない系統 = `isSClassItem()`。金の S バッジはこれで付く)で、S級系統のレア度はミシック。
  ...
  - **品質ボーナス(レア度スキル)**(定義は `EQUIP_QUALITY`。アーチャー伝説2の品質スキル方式):
    ...
    **ステータス加算の段はコモン・レアの2段だけ**で、エピック以上の段は置かない
    (エピックからは下の部位固有効果が乗る。高レアは素の数値の上積みではなく挙動で差を付ける)。
  ...
  - **部位固有効果**(土台の数値は `CONFIG.equip.slotFx`、効果キーは `EQUIP_FX_KEYS` =
    ...
    **固有効果はレア度の段の梯子で持つ**(`EQUIP_SERIES[se].fx[slot] = { key, tiers }`。`tiers` は
    `EQUIP_FX_TIERS` = コモン→ミシックの5段の加算値): アイテムは自分のレア度以下の段を全部持ち、
    品質スキルと同じく `qualityBonuses()` が段ごとの行に展開する(0 の段は行にしない)ので、
    同じ系統でもレア度が上がるほど効果行が増えて強くなる。**固有効果はエピックで初めて発動**
    (コモン・レアの段は 0)。狩人(コモン)・銀狼(レア)は基礎ステータス+系統ボーナスだけで、
    固有効果は星詠(エピック)から。通常系統は `fxOf` で対になるS級系統の
    梯子を借りて自分のレア度で止まる = 値はS級側にだけ書き、下位互換が構造として保証される。
    アイテム側には固有効果のフィールドを書かない(合計は `itemFxTotal(id)`)。
  ...
  - UI は**下部常設タブバー**(`#title-tabbar`。定義は `TITLE_TABS` 配列でデータ駆動、
    ...
    表示のみ)+**所持装備のアイコンタイルのグリッド**(アーチャー伝説風。レアリティ順→
    スロット順の4列。タイルはレアリティ色の枠と背景(`tileRarityStyle()`。CSS変数だけでは
    α付きの派生色を作れないのでJSでグラデをインライン指定)+部位ラベル+Lv+S級の金バッジ+
    装備中タグ)。タイルをタップすると**詳細ポップアップ**(`openEquipDetail()`。大アイコン・
    メイン効果・品質スキル(段ごとのレア度色の◆)・系統ボーナス(発動段は✓、未発動は灰色)・
    装備/強化ボタン。強化してもポップアップは開いたまま更新され、背面タップ・✕・タブ切替で閉じる)
  - 開発パネルの火力表(`renderDbgPower()`)冒頭に装備欄あり。装備タブでコイン付与・全装備付与・
    セーブ初期化ができ、その下に**未所持も含む全装備カタログ**(レアリティ順→スロット順。
    系統・所持状況・効果・品質つきで、行のボタンから1個付与/素材+5)を一覧する。
    ゲーム内の装備画面は所持品しか出さないので、追加したアイテムの定義確認はここで行う。**無料ガチャのトグル**(ON中はコイン消費なしで無限に引ける。リロードでOFF)は
...
- **開発画面パネル**: プレイ中(またはタイトル)に HUD の 🛠ボタン or キー K で開くタブ式のパネル。
  装備画面のヘッダーにも 🛠 開発ボタンがあり、こちらは**装備タブを表示した状態で**開く
  (`switchDbgTab('equip')`。装備の付与・カタログは装備画面で使う機能のため)。
