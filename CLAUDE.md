...
- **弓の構え・発射モーション**(数値は `CONFIG.player.bow`): 発射サイクル(攻撃速度スキル込みの
  ...
  微かな震え+仰け反り)→ 放つ(`firePlayerShot` 直後に `anim.release = 1`)→ フォロースルー
  (引き手が外へ流れ、弓手が跳ね上がる)」を繰り返す。`updatePlayer()` が `anim.drawGoal` を
  サイクル進行から毎フレーム計算し、`updatePlayerAnimation()` が追従させる。
  弦の折れの深さ(`drawDepth`)は引き量から直接決める — デフォルメの短い腕では手の実座標の
  後退が 0.01 程度しかなく弦が張ったままに見えるため。番え矢は引き切りでも矢先が弓の前に
  残る長さにしてあり(俯瞰では大きな頭に隠れやすい)、リリースで即座に消えて発射された矢と繋がる。
  引き絞りの仰け反りは奥向きに構えたとき矢先を頭の陰から出す役も持つ
